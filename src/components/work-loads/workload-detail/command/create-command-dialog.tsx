...
import { useCommands, CommandCreate } from "@/hooks/command/use-commands";
...
export const CreateCommandDialog: React.FC<CreateCommandDialogProps> = ({
  ruleId,
  open,
  onOpenChange,
  onSuccess,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>Tạo Command Mới</DialogTitle>
          <DialogDescription>
            Tạo command mới cho rule. Điền thông tin cần thiết và nhấn lưu.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-sm text-muted-foreground">
                Command sẽ được kích hoạt ngay sau khi tạo
              </p>
            </div>
            <Switch
              id="is_active"
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            <Button
              type="submit"
              disabled={
                loading || !formData.os_version || !formData.command_text.trim()
              }
            >
              <Save className="h-4 w-4 mr-2" />
              {loading ? "Đang tạo..." : "Tạo Command"}
            </Button>
          </div>
        </form>
      </DialogContent>
