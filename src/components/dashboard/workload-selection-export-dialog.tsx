...
export const WorkloadSelection: React.FC<WorkloadSelectionProps> = ({
  ...
  const { workloads, loading: loadingWorkloads, fetchWorkloads } = useWorkloads();
  ...
  const filteredWorkloads = React.useMemo(() => {
    ...
    return workloads.filter(workload =>
      workload.name.toLowerCase().includes(workloadSearchTerm.toLowerCase()) ||
      workload.description?.toLowerCase().includes(workloadSearchTerm.toLowerCase())
    );
      ...
      <div className="col-span-3 space-y-3">
        ...
        <div className="min-h-[40px] p-2 border rounded-md bg-gray-50">
          {selectedWorkloadObjects.length === 0 ? (
            <span className="text-muted-foreground text-sm">Chưa chọn workload nào</span>
          ) : (
              ...
              <div className="p-2">
                {loadingWorkloads ? (
                  ...
                  <div className="text-center py-4 text-sm text-gray-500">
                    {workloadSearchTerm ? "Không tìm thấy workload" : "Không có workload nào"}
                  </div>
                ) : (
                  filteredWorkloads.map((workload) => (
                    <div key={workload.id} className="flex items-center space-x-2 py-1">
                      <Checkbox
